...
import { getWarehouseBatchesSampleData, type Batch } from '@/data/warehouses/warehouse-batches.data';

export const WarehouseBatches = (): JSX.Element => {
  ...
  const [batches, setBatches] = useState<Batch[]>(() => getWarehouseBatchesSampleData());
  ...
  const handleMenuClick = (e: React.MouseEvent, batch: Batch) => {
    e.stopPropagation();
    const rect = e.currentTarget.getBoundingClientRect();
    const menuWidth = 192;
    const menuHeight = batch.isActive ? 140 : 100;
  ...
  const handleEdit = () => {
    if (selectedBatch) {
      navigate(`/warehouses/batches/edit/${selectedBatch.id}`);
    }
  };
    ...
    if (selectedBatch) {
      setBatches(batches.map(batch =>
        batch.id === selectedBatch.id
          ? { ...batch, isActive: actionType === 'activate' }
          : batch
      ));
    }
  ...
  const filteredBatches = batches.filter(batch =>
    ...
    batch.originalItem.toLowerCase().includes(searchQuery.toLowerCase())
  );
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between gap-2 mb-4" dir="rtl">
          <div className="relative flex-1 max-w-[450px]">
            <Input
              type="text"
              placeholder="ابحث من هنا (الكود، الاسم، البند ...)"
              value={searchQuery}
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => navigate('/warehouses/batches/create')}
            ...
            <table className="w-full" dir="rtl">
              <thead>
                <tr className="bg-[#F1F5F980] border-b border-gray-200">
                  ...
                  <th className="px-4 py-3 text-right text-sm font-medium text-[#093738]">اسم الدفعة</th>
                  <th className="px-4 py-3 text-right text-sm font-medium text-[#093738]">نوع الصنف</th>
                  <th className="px-4 py-3 text-right text-sm font-medium text-[#093738]">البند الأصلي</th>
                  <th className="px-4 py-3 text-right text-sm font-medium text-[#093738]">تاريخ التصنيع</th>
                  <th className="px-4 py-3 text-right text-sm font-medium text-[#093738]">تاريخ الانتهاء</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredBatches.map((batch) => (
                  <tr key={batch.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-900">{batch.name}</td>
                    <td className="px-4 py-3 text-sm">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        batch.itemType === 'inventory' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800'
                      }`}>
                        {batch.itemType === 'inventory' ? 'مخزني' : 'خدمي'}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-900">{batch.originalItem}</td>
                    <td className="px-4 py-3 text-sm text-gray-900">{batch.manufacturingDate}</td>
                    <td className="px-4 py-3 text-sm text-gray-900">{batch.expiryDate}</td>
                    <td className="px-4 py-3 text-sm">
                      {batch.isActive ? (
        ...
        {/* Dropdown Menu */}
        {openMenuId && menuPosition && selectedBatch && (
          <div
            ref={menuRef}
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" dir="rtl">
            <div className="bg-white rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
                <h2 className="text-xl font-semibold text-[#093738]">تفاصيل الدفعة</h2>
                <button
              ...
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-900">{selectedBatch.code}</p>
                  </div>
                  <div>
                    <label className="text-sm font-medium text-gray-700">اسم الدفعة</label>
                    <p className="mt-1 text-sm text-gray-900">{selectedBatch.name}</p>
                  </div>
                  <div>
                    <label className="text-sm font-medium text-gray-700">نوع الصنف</label>
                    <p className="mt-1 text-sm text-gray-900">{selectedBatch.itemType === 'inventory' ? 'مخزني' : 'خدمي'}</p>
                  </div>
                  <div>
                    <label className="text-sm font-medium text-gray-700">البند الأصلي</label>
                    <p className="mt-1 text-sm text-gray-900">{selectedBatch.originalItem}</p>
                  </div>
                  <div>
                    <label className="text-sm font-medium text-gray-700">تاريخ التصنيع</label>
                    <p className="mt-1 text-sm text-gray-900">{selectedBatch.manufacturingDate}</p>
                  </div>
                  <div>
                    <label className="text-sm font-medium text-gray-700">تاريخ انتهاء الصلاحية</label>
                    <p className="mt-1 text-sm text-gray-900">{selectedBatch.expiryDate}</p>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-white rounded-lg w-full max-w-md p-6">
              <div className="flex flex-col items-center text-center">
                ...
                <h3 className="text-lg font-semibold text-gray-900 mb-2">
                  {actionType === 'activate' ? 'تفعيل الدفعة' : 'إلغاء تفعيل الدفعة'}
                </h3>
                <p className="text-sm text-gray-600 mb-6">
                  {actionType === 'activate'
                    ? `هل أنت متأكد من تفعيل "${selectedBatch.name}"؟`
                    : `هل أنت متأكد من إلغاء تفعيل "${selectedBatch.name}"؟`
                  }
                </p>

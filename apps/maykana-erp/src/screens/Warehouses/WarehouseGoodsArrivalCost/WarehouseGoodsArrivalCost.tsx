...
import { getWarehouseGoodsArrivalCostSampleData, type GoodsArrivalCost } from '@/data/warehouses/warehouse-goods-arrival-cost.data';

export const WarehouseGoodsArrivalCost = (): JSX.Element => {
  const navigate = useNavigate();
  const [searchQuery, setSearchQuery] = useState('');
  const [openMenuId, setOpenMenuId] = useState<string | null>(null);
  const [menuPosition, setMenuPosition] = useState<{ top: number; left: number } | null>(null);
  const [selectedItem, setSelectedItem] = useState<GoodsArrivalCost | null>(null);
  const [isDetailsModalOpen, setIsDetailsModalOpen] = useState(false);
  const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);
  const [actionType, setActionType] = useState<'activate' | 'deactivate'>('deactivate');
  const menuRef = useRef<HTMLDivElement>(null);

  const [items, setItems] = useState<GoodsArrivalCost[]>(() => getWarehouseGoodsArrivalCostSampleData());
  ...
  const handleMenuClick = (e: React.MouseEvent, item: GoodsArrivalCost) => {
    e.stopPropagation();
  ...
  const handleEdit = () => {
    if (selectedItem) {
      navigate(`/warehouses/goods-arrival-cost/edit/${selectedItem.id}`);
    }
  };
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between gap-2 mb-4" dir="rtl">
          <div className="relative flex-1 max-w-[450px]">
            <Input
              type="text"
              placeholder="ابحث من هنا (الكود، الشركة، المورد ...)"
              value={searchQuery}
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => navigate('/warehouses/goods-arrival-cost/create')}
        ...
        <Card className="bg-white rounded-lg border border-gray-200 overflow-hidden">
            ...
            <table className="w-full" dir="rtl">
              <thead>
                <tr className="bg-[#F1F5F980] border-b border-gray-200">
                  ...
                  <th className="px-4 py-3 text-right text-sm font-medium text-[#093738]">الشركة</th>
                  <th className="px-4 py-3 text-right text-sm font-medium text-[#093738]">المورد</th>
                  <th className="px-4 py-3 text-right text-sm font-medium text-[#093738]">الإجمالي قبل الضريبة</th>
                  <th className="px-4 py-3 text-right text-sm font-medium text-[#093738]">الإجمالي بعد الضريبة</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredItems.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-900">{item.totalBeforeTax.toLocaleString()} ر.س</td>
                    <td className="px-4 py-3 text-sm text-gray-900">{item.totalAfterTax.toLocaleString()} ر.س</td>
          ...
          {filteredItems.length === 0 && (
            <div className="text-center py-12 text-gray-500">
              لا توجد قسائم تكلفة وصول بضاعة
            </div>
          )}
        </Card>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" dir="rtl">
            <div className="bg-white rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
                <h2 className="text-xl font-semibold text-[#093738]">تفاصيل قسيمة تكلفة وصول البضاعة</h2>
                <button
              ...
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-900">{selectedItem.company}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    <label className="text-sm font-medium text-gray-700">الإجمالي قبل الضريبة</label>
                    <p className="mt-1 text-sm text-gray-900">{selectedItem.totalBeforeTax.toLocaleString()} ر.س</p>
                  </div>
                  <div>
                    <label className="text-sm font-medium text-gray-700">الإجمالي بعد الضريبة</label>
                    <p className="mt-1 text-sm text-gray-900">{selectedItem.totalAfterTax.toLocaleString()} ر.س</p>
                  </div>
                  <div>
                    <label className="text-sm font-medium text-gray-700">التاريخ</label>
                    <p className="mt-1 text-sm text-gray-900">{formatDate(selectedItem.date)}</p>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-white rounded-lg w-full max-w-md p-6">
              <div className="flex flex-col items-center text-center">
                ...
                <h3 className="text-lg font-semibold text-gray-900 mb-2">
                  {actionType === 'activate' ? 'تفعيل القسيمة' : 'إلغاء تفعيل القسيمة'}
                </h3>
                <p className="text-sm text-gray-600 mb-6">
                  {actionType === 'activate'
                    ? `هل أنت متأكد من تفعيل "${selectedItem.code}"؟`
                    : `هل أنت متأكد من إلغاء تفعيل "${selectedItem.code}"؟`
                  }
                </p>

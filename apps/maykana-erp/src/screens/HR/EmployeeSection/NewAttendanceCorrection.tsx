...
export const NewAttendanceCorrection: React.FC = () => {
  ...
  const getBackToList = () => '/hr/employee/leaves-attendance?tab=attendance-correction';

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log('Attendance Correction:', formData);
      ...
      <div style={{ fontFamily: 'IBM Plex Sans Arabic, sans-serif' }}>
        ...
        <div className="flex items-center justify-between mb-6 bg-white p-4 rounded-xl border border-[#e2e2e2]">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-medium text-gray-900">{t('hr.emp_attendance_correction')}</h1>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            ...
            {otherInfoOpen && (
              <div className="grid grid-cols-4 gap-6">
                ...
                {/* Correct Entry Time */}
                <div className="flex flex-col gap-1.5">
                  <label className="text-base text-gray-600 text-right">{t('hr.correct_entry_time')}</label>
                  <input
                    type="time"
                    value={formData.correctEntryTime}
                    onChange={(e) => setFormData({ ...formData, correctEntryTime: e.target.value })}
                    className="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#093738] focus:border-transparent text-right"
                    dir="rtl"
                    placeholder={t('hr.correct_entry_time')}
                    required
                  />
                </div>
                ...
                {/* Correct Exit Time */}
                <div className="flex flex-col gap-1.5">
                  <label className="text-base text-gray-600 text-right">{t('hr.correct_exit_time')}</label>
                  <input
                    type="time"
                    value={formData.correctExitTime}
                    onChange={(e) => setFormData({ ...formData, correctExitTime: e.target.value })}
                    className="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#093738] focus:border-transparent text-right"
                    dir="rtl"
                    placeholder={t('hr.correct_exit_time')}
                    required
                  />
                </div>

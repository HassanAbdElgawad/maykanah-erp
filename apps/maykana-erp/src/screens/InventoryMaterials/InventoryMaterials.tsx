...
import type { InventoryMaterial } from '../../data/inventory-materials.data';

export const InventoryMaterials = (): JSX.Element => {
  const navigate = useNavigate();
  const { t, language } = useLanguage();
  const [searchTerm, setSearchTerm] = useState('');
  const { materials } = useInventoryMaterialsData();

  const getStatusBadge = (status: InventoryMaterial['status']) => {
    const statusConfig = {
      available: {
        bg: 'bg-[#2cc28d26]',
        text: 'text-[#2cc28d]',

...
interface MaterialRequestsListFormProps {
...
export function MaterialRequestsListForm({ mode: propMode }: MaterialRequestsListFormProps) {
  ...
  const [requestDate, setRequestDate] = useState('10/11/2025');
  const [requestTime, setRequestTime] = useState('09:30');
  ...
  const totalInvoice = requestItems.reduce((sum, item) => sum + item.totalPrice, 0);
  const totalTax = requestItems.reduce((sum, item) => sum + item.taxAmount, 0);
        ...
        <div className="p-6">
          ...
          <div className="mb-6 flex items-center justify-between">
            <div className="flex items-center gap-4">
              <button
                onClick={() => navigate('/purchases/material-requests-list')}
            ...
            {expandedSections.information && (
              <div className="px-6 pb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  {isReadOnly ? (
                    <div className="p-2 bg-gray-50 rounded border">{`${requestTime} - ${requestDate}`}</div>
                  ) : (

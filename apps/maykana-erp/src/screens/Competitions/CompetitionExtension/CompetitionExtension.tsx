...
export function CompetitionExtension() {
  ...
  const handleCreateNew = () => {
    navigate('/competitions/competition-extension/add');
  };

  const handleEdit = (id: string) => {
    navigate(`/competitions/competition-extension/edit/${id}`);
  };

  const mockData: CompetitionExtension[] = [
    {
      id: '1',
  ...
  const actionButtons = [
    {
      icon: Pencil,
      label: t('common.edit'),
      onClick: (row: CompetitionExtension) => handleEdit(row.id),
      color: 'blue' as const,
    },
  ];
      ...
      >
        ...
        <div className="flex items-center justify-between bg-white p-4 rounded-xl border border-[#e2e2e2]">
          ...
          <div className="flex gap-2">
            ...
            >
              {t('competition_extension.add_new')}
            </button>
          </div>
        </div>
            ...
            <div className="grid grid-cols-4 gap-4">
              <div className="flex flex-col gap-1.5">
                <Label className="text-sm font-medium text-gray-700">
                  {t('competition_extension.entity_type')}
                </Label>
                <select className="w-full h-[42px] px-3 border border-[#e2e2e2] rounded-lg bg-white">
                  <option>{t('competition_extension.entity_type')}</option>
                </select>
              </div>
              <div className="flex flex-col gap-1.5">
                <Label className="text-sm font-medium text-gray-700">
                  {t('competition_extension.source')}
                </Label>
                <select className="w-full h-[42px] px-3 border border-[#e2e2e2] rounded-lg bg-white">
                  <option>{t('competition_extension.source')}</option>
                </select>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-xl border border-[#e2e2e2] overflow-hidden">
          <AdvancedTable
            ...
            columns={[
              {
                key: 'name',
                label: t('competition_extension.name'),
              },
              {
                key: 'recordNumber',
                label: t('competition_extension.record_number'),
              },
              {
                key: 'entityType',
                label: t('competition_extension.entity_type'),
              },
              {
                key: 'source',
                label: t('competition_extension.source'),
              },
              {
                key: 'recordDate',
                label: t('competition_extension.record_date'),
              },
              {
                key: 'accountNumber',
                label: t('competition_extension.account_number'),
              },
            ]}
        ...
        <div className="flex items-center justify-between px-8 py-4">
          <Button variant="outline" className="px-6 py-2 border-[#e7e7e7]">
            {t('competition_extension.previous')}
          </Button>

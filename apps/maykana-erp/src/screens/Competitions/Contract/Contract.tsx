...
export function Contract() {
  ...
  const handleEdit = (id: number) => {
    navigate(`/competitions/contract/edit/${id}`);
  };
      ...
      >
        ...
        <div className="flex items-center justify-between bg-white p-4 rounded-xl border border-[#e2e2e2]">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={() => navigate('/competitions/contract/add')}
              className={buttonClasses.primary}
            >
              {t('competitions.contract.add_new')}
            </button>
          </div>
        </div>
        ...
        {showFilters && (
          <div className="bg-white p-6 rounded-xl border border-[#e2e2e2] animate-fade-in">
            <div className="grid grid-cols-4 gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                <Label className="text-sm font-medium text-gray-700">
                  {t('competitions.contract.supplier_type')}
                </Label>
                <Input
                  type="text"
                  placeholder={t('competitions.contract.supplier_type')}
        ...
        <div className="bg-white rounded-xl border border-[#e2e2e2] overflow-hidden">
          <AdvancedTable
            ...
            columns={[
              {
                key: 'supplier',
                label: t('competitions.contract.supplier'),
              },
              {
                key: 'contractNumber',
                label: t('competitions.contract.contract_number'),
              },
              {
                key: 'supplierType',
                label: t('competitions.contract.supplier_type'),
              },
              {
                key: 'project',
                label: t('competitions.contract.project'),
              },
              {
                key: 'confirmationDate',
                label: t('competitions.contract.confirmation_date'),
              },
              {
                key: 'competitionNumber',
                label: t('competitions.contract.competition_number'),
              },
            ]}

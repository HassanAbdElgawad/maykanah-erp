...
export function CompetitionLaunch() {
  ...
  const handleCreateNew = () => {
    navigate('/competitions/competition-launch/add');
  };

  const handleEdit = (id: string) => {
    navigate(`/competitions/competition-launch/edit/${id}`);
  };

  const mockData: CompetitionLaunch[] = [
    {
      id: '1',
  ...
  const actionButtons = [
    {
      icon: Pencil,
      label: t('common.edit'),
      onClick: (row: CompetitionLaunch) => handleEdit(row.id),
      color: 'blue' as const,
    },
  ];
            ...
            <div className="grid grid-cols-4 gap-4">
              <div className="flex flex-col gap-1.5">
                <Label className="text-sm font-medium text-gray-700">
                  {t('competition_launch.entity_type')}
                </Label>
                <select className="w-full h-[42px] px-3 border border-[#e2e2e2] rounded-lg bg-white">
                  <option>{t('competition_launch.entity_type')}</option>
                </select>
              </div>
              <div className="flex flex-col gap-1.5">
                <Label className="text-sm font-medium text-gray-700">
                  {t('competition_launch.source')}
                </Label>
                <select className="w-full h-[42px] px-3 border border-[#e2e2e2] rounded-lg bg-white">
                  <option>{t('competition_launch.source')}</option>
                </select>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-xl border border-[#e2e2e2] overflow-hidden">
          <AdvancedTable
            ...
            columns={[
              {
                key: 'name',
                label: t('competition_launch.name'),
              },
              {
                key: 'recordNumber',
                label: t('competition_launch.record_number'),
              },
              {
                key: 'entityType',
                label: t('competition_launch.entity_type'),
              },
              {
                key: 'source',
                label: t('competition_launch.source'),
              },
              {
                key: 'recordDate',
                label: t('competition_launch.record_date'),
              },
              {
                key: 'accountNumber',
                label: t('competition_launch.account_number'),
              },
            ]}

...
> **للفهم الكامل للهيكل:** راجع `docs/DATA_ARCHITECTURE.md` — يشرح الطبقات الثلاث (Data → Hook → Screen) والطبقة المركزية للتكامل (`@/config/integration-status.config.ts`).
...
## كيف نتعامل مع البيانات بعد الفصل؟

### 1. هيكل ملف الـ Data

```typescript
// @/data/reports/warehouses/inventory-balance.data.ts
...
// 2. دوال لجلب البيانات (للقوائم)
export const getInventoryBalanceSampleData = (): InventoryBalanceItem[] => [
  { id: '1', itemCode: 'SR-2500096', name: 'شركة افاق الخليج', ... },
  // ...
];

// 3. للصفحات Edit/View: دالة جلب حسب ID
export const getUnitById = (id: string) => mockUnits[id] ?? null;
```

### 2. كيف يستخدمها الـ Screen

```tsx
// قبل (بيانات داخلية)
const [data] = useState<InventoryBalanceItem[]>([
  { id: '1', itemCode: 'SR-2500096', ... },
]);

// بعد (استيراد من ملف الـ data)
import { getInventoryBalanceSampleData } from '@/data/reports/warehouses/inventory-balance.data';

const data = getInventoryBalanceSampleData();
// أو مع useState إذا تحتاج تحديث:
const [data, setData] = useState(getInventoryBalanceSampleData());
```

### 3. أنماط شائعة

| النوع | في ملف الـ Data | في الـ Screen |
|-------|-----------------|---------------|
| **قائمة (List)** | `getXxxData(): Item[]` | `const data = getXxxData()` |
| **بحث بالـ ID (Edit/View)** | `getXxxById(id: string)` أو `mockXxx: Record<string, Item>` | `const item = getXxxById(id)` |
| **قيم افتراضية للفورم** | `getInitialFormData(): FormData` | `useState(getInitialFormData())` |
| **خيارات ثابتة** | `MOVEMENT_TYPES`, `STATUS_OPTIONS` | `import { MOVEMENT_TYPES } from '@/data/...'` |

### 4. عند ربط الـ API لاحقاً

فقط غيّر **محتوى** ملف الـ Data، دون تعديل الـ Screen:
...
// بعد: API call
export const getInventoryBalanceSampleData = async (): Promise<InventoryBalanceItem[]> => {
  const res = await fetch('/api/reports/inventory-balance');
  return res.json();
};
```

> الـ Screen يستمر في استدعاء نفس الدالة؛ يمكن استخدام **hooks** مثل `useQuery` لاحقاً للتعامل مع الـ async.

### 5. التصدير من data/index.ts

أضف تصدير ملف الـ data الجديد في `src/data/index.ts` إذا أردت استيراده من `@/data`:
...
| 31 | Settings | MaintenanceTeamView.tsx | `mockTeams` | `@/data/settings/maintenance-team.data.ts` (موجود - تحقق) | ✓ |
